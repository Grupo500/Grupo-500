...
0. **Identificarse es requisito, no cortesía** (decisión de Hotman, 18-ago):
   si `git config user.name` es el genérico "Grupo500" o está vacío, la sesión
   NO ejecuta ninguna tarea sobre el proyecto — ni ediciones ni comandos — hasta
   que la persona se identifique con `git config --local user.name "SuNombre"`
