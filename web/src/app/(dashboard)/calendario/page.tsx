...
/* ─── Helpers ────────────────────────────────────────────────────────────── */
function esVencidaCuota(c: CuotaCalendario) {
...
function generarMensajeWA(cuota: CuotaCalendario) {
  ...
  const fecha = new Date(cuota.fechaVencimiento).toLocaleDateString('es-CO', { day: '2-digit', month: 'long' })
  const msg = encodeURIComponent(
    `Hola ${nombre}, le recordamos que la cuota de *${cuota.financiamiento.estudiante.nombre}* por *${formatCOP(cuota.monto)}* vence el *${fecha}*. Por favor realizar el pago a tiempo. Gracias — Grupo 500 🎓`
  )
                    ...
                    <div className="flex items-start justify-between mb-1.5">
                      <div className="min-w-0">
                        <p className="text-[13px] font-semibold text-on-surface truncate">
                          {cuota.financiamiento.estudiante.nombre}
                        </p>
                        {cuota.financiamiento.estudiante.acudiente && (
                          <p className="text-[11px] text-on-surface-variant">
                            {cuota.financiamiento.estudiante.acudiente.nombre}
                          </p>
                        )}
