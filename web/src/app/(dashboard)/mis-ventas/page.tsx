...
import { ChevronLeft, ChevronRight, Search, Repeat2, Loader2, Receipt } from 'lucide-react'

// Cifras tabulares: los montos se comparan en columna, así que necesitan
// ancho fijo por dígito. Poppins (la fuente global) no lo garantiza.
...
export default function MisVentasPage() {
  ...
  const { data: resumen, isLoading: cargandoResumen } = useQuery<{ data: Resumen }>({
    queryKey: ['mis-ventas-resumen', offset],
    queryFn: () => fetcher(`/reportes/mis-ventas?${rangoQS}`),
    enabled: !!token,
  })

  const { data: lista, isLoading: cargandoLista } = useQuery<{ data: Venta[]; meta?: { total: number; totalPages: number } }>({
    queryKey: ['mis-ventas-lista', offset, busqueda, cursoId, pagina],
    queryFn: () =>
      fetcher(
        `/pagos?porFechaPago=true&estado=PAGADO&${rangoQS}&page=${pagina}&limit=20` +
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="Mis ventas"
        subtitle="Cada venta que cerraste, con su comisión"
      />
