 ...
 *
 * Se arma en el navegador con jsPDF —la misma librería de los certificados— y
 * no en el servidor: así la persona la ve al instante y lo que se archiva en
 * Drive es exactamente el archivo que descargó, no una segunda versión
 * dibujada por otro código que podría desviarse con el tiempo.
 *
 * Los datos vienen de dos sitios que ya existen y no se vuelven a pedir: los
 * fijos del perfil (nombre, cédula, banco, firma) y los del propio trabajo
 * (fecha, concepto, valor, periodo).
 */
...
async function firmaComoDataUri(url: string): Promise<string | null> {
  try {
    ...
    return await new Promise(ok => {
      ...
      lector.onloadend = () => ok(lector.result as string)
      lector.onerror = () => ok(null as any)
...
export async function generarCuentaDeCobro(persona: DatosPersona, cobro: DatosCobro) {
  ...
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(10.5)
  const periodo = cobro.periodoDesde && cobro.periodoHasta
    ? ` Servicio prestado entre el ${enLetra(cobro.periodoDesde)} y el ${enLetra(cobro.periodoHasta)}.`
    : ''
  const lineas = doc.splitTextToSize(cobro.concepto + '.' + periodo, util)
  ...
  return { blob, base64, archivo: nombreArchivo(persona, cobro) }
}
