...
const cobros = [
  ...
  { nombre: 'Pedro Ramírez',    monto: 200000, fecha: new Date(Date.now() + 7  * 86400000).toISOString() },
]
    ...
    <div className="card p-5 flex flex-col h-72">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-[13px] font-semibold text-on-surface truncate">{c.nombre}</p>
      ...
      <div className="mt-3 pt-3 border-t border-[var(--outline-variant)] flex items-center justify-between">
        ...
        <span className="text-[13px] font-bold text-on-surface tabular">
          {formatCOP(cobros.reduce((s, c) => s + c.monto, 0))}
        </span>
      </div>
    </div>
  )
