...
// ---------------------------------------------------------------------------
// Diagnóstico temporal (solo ADMIN): prueba de humo contra el endpoint de
// afiliados de Hotmart, antes de construir el módulo "Enlaces". Confirma si
// responde con datos o si las credenciales de productor no tienen permiso.
// Quitar cuando quede resuelto.
// ---------------------------------------------------------------------------
export async function diagnosticoAfiliados(req: Request, res: Response) {
  const { cursoId } = req.query

  const curso = cursoId
    ? await prisma.curso.findUnique({ where: { id: String(cursoId) } })
    : await prisma.curso.findFirst({ where: { hotmartProductId: { not: null }, activo: true } })

  if (!curso?.hotmartProductId) {
    return res.status(404).json({ success: false, error: 'No se encontró un curso con hotmartProductId configurado' })
  }
  ...
  const url = `https://api-hot-connect.hotmart.com/affiliation/rest/v2/products/${curso.hotmartProductId}/affiliates`
  const apiRes  = await fetch(url, { headers: { Authorization: `Bearer ${token}` } })
  const rawText = await apiRes.text()

  let body: unknown
  try { body = JSON.parse(rawText) } catch { body = rawText.slice(0, 2000) }

  logger.info(`[Hotmart] Diagnóstico afiliados — status ${apiRes.status} — curso ${curso.nombre} (${curso.hotmartProductId})`)

  return ApiResponse.success(res, {
    curso: { id: curso.id, nombre: curso.nombre, hotmartProductId: curso.hotmartProductId },
    url,
