...
// ---------------------------------------------------------------------------
// Backfill: rellenar teléfonos de estudiantes que ya pagaron, desde el
// historial de ventas de Hotmart. Solo llena teléfonos vacíos (no sobrescribe).
// ---------------------------------------------------------------------------
export async function sincronizarHistorialVentas(_req: Request, res: Response) {
  const clientId     = process.env.HOTMART_CLIENT_ID
  const clientSecret = process.env.HOTMART_CLIENT_SECRET
  if (!clientId || !clientSecret) {
    return res.status(503).json({ success: false, error: 'Hotmart API no configurada' })
  ...
  const startDate = Date.now() - DOS_ANIOS_MS
  ...
  do {
    ...
    if (!apiRes.ok) {
      logger.error(`[Hotmart] Backfill — error historial: ${apiRes.status} ${text.slice(0, 300)}`)
      return res.status(apiRes.status).json({ success: false, error: 'Error al consultar el historial de Hotmart' })
    }
    let json: { items?: any[]; page_info?: { next_page_token?: string } } = {}
    ...
    pageToken = json.page_info?.next_page_token
    vueltas++
  } while (pageToken && vueltas < 50)

  // Muestra de la estructura real (para verificar qué campos trae la API)
  const muestra = allItems.slice(0, 3).map(it => it?.buyer ?? {})
  logger.info(`[Hotmart] Backfill — ${allItems.length} ventas. Muestra buyer: ${JSON.stringify(muestra)}`)
  ...
  for (const item of allItems) {
    ...
    const phone: string | undefined = buyer.checkout_phone ?? buyer.phone ?? buyer.cellphone
    ...
    if (!est) { sinEstudiante++; continue }
    if (est.telefono) continue          // ya tiene teléfono, no tocar
    if (!phone)       { sinTelefono++; continue }

    await prisma.estudiante.update({ where: { id: est.id }, data: { telefono: phone } })
    actualizados++
  }

  logger.info(`[Hotmart] Backfill completado — actualizados: ${actualizados}, sin tel: ${sinTelefono}, sin estudiante: ${sinEstudiante}, sin email: ${sinEmail}`)
  return ApiResponse.success(res, {
    totalVentas: allItems.length,
    actualizados,
    sinTelefono,
    sinEstudiante,
    sinEmail,
